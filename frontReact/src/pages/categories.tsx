    ...
    const getCategories = async () => {
        ...
        try {
            const { error, data } = await supabase
                .from('Categorias')
                .select('*')
                .eq('estado', true) // Solo mostrar categorías activas
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (error) {
                ...
                setToastConfig({ show: true, message: 'Categoría eliminada correctamente' });
            }
        } catch (err) {
                    ...
                    <div className="bg-white border rounded-xl shadow-sm border-gray-200">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {isLoading ? (
                                        <tr>
                                            <td colSpan={3} style={{ height: TABLE_MIN_HEIGHT }}>
                                        ...
                                        <>
                                                ...
                                                <tr key={category.id} className="hover:bg-gray-50 transition-colors group">
                                                    ...
                                                    <td className={`p-5 text-right relative ${openActionMenuId === category.id ? "z-20" : "z-0"}`}>
                                                        ...
                                                        {openActionMenuId === category.id && (
                                                            <div
                                                                ref={menuRef}
                                                                className={`absolute right-8 w-40 bg-white rounded-lg shadow-lg border border-gray-100 z-50 py-1 animation-fade-in ${index >= paginatedCategories.length - 2 ? "bottom-8 origin-bottom-right" : "top-8 origin-top-right"
                                                                    }`}
                                            ...
                                            {Array.from({ length: Math.max(0, MIN_ROWS - paginatedCategories.length) }).map((_, i) => (
                        ...
                        <div className="p-5 border-t border-gray-100 flex items-center justify-between">
                            <div className="text-sm text-gray-600 font-medium">
                                Mostrando <span className="font-bold text-gray-900">{filteredCategories.length > 0 ? indexOfFirstItem + 1 : 0}</span> a <span className="font-bold text-gray-900">{Math.min(indexOfLastItem, filteredCategories.length)}</span> de <span className="font-bold text-gray-900">{filteredCategories.length}</span>
                            </div>
